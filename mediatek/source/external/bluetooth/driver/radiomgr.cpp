    ...
    )
{
    ...
    if(bt_send_data(hComPortFile, HCI_CMD, len + 4) < 0){
        printf("Write HCI command fails errno %d\r\n", errno);
        return FALSE;
    }
    ...
    )
{
    ...
    if(bt_receive_data(hComPortFile, &type, 1) < 0){
        printf("Read packet header fails\r\n");
        return FALSE;
    }
    ...
    *pPacketType = type;
    ...
    switch(type){
        case 1: //cmd
            ...
            {
                printf("Read cmd header fails %d\r\n", errno);
                return FALSE;
            }
        ...
        case 2: //ACL data
            ...
            {
                printf("Read ACL header fails %d\r\n", errno);
                return FALSE;
            }
        ...
        case 3: //SCO data
            ...
            {
                printf("Read SCO header fails %d\r\n", errno);
                return FALSE;
            }
        ...
        case 4: //event
            ...
            {
                printf("Read event header fails %d\r\n", errno);
                return FALSE;
            }
    ...
    )
{
    ...
    if(dwMaxBufSz == 0){
        printf("Read packet output is too short \r\n");
        return FALSE;
    }
...
START:
    if(BT_ReadPacketHeader(
        ...
        if(i < 7){
            printf("Get %02x for packet type\n", packetType);
            i++;
            goto START;
        }
        else{
    ...
    if(packetType == 1){
        if(dwMaxBufSz < (4 + remainLen)){
            printf("Read command packet output is too short \r\n");
            return FALSE;
        }
        ...
        {
            printf("Read remain packet fails %d \r\n", errno);
            return FALSE;
        }    
    ...
    if(packetType == 2){
        if(dwMaxBufSz < (5 + remainLen)){
            printf("Read acl packet output is too short \r\n");
            return FALSE;
        }
    ...
    if(packetType == 3){
        if(dwMaxBufSz < (4 + remainLen)){
            printf("Read sco packet output is too short \r\n");
            return FALSE;
        }
    ...
    if(packetType == 4){
        if(dwMaxBufSz < (3 + remainLen)){
            printf("Read event packet output is too short \r\n");
            return FALSE;
        }
    ...
    )
{
    ...
    if(bt_send_data(hComPortFile, HCI_PRE_CONFIG, sizeof(HCI_PRE_CONFIG)) < 0){	
        printf("Write pre config command fails\r\n");
        return FALSE;
    }
    ...
    if(BT_ReadExpectedEvent(
        ...
        printf("Read complete event fails\r\n");
        return FALSE;
    }
    ...
    )
{
    ...
    printf("Host baud %d HCI_VS_UART_CONFIG %02x %02x %02x %02x %02x %02x\r\n",
        (int)dwHostBaud,
    ...
    if(bt_send_data(hComPortFile, HCI_VS_UART_CONFIG, sizeof(HCI_VS_UART_CONFIG))< 0){	
        printf("Write UART config command fails\r\n");
        return FALSE; 	
    }
    ...
    if(BT_ReadExpectedEvent(
        ...
        printf("Read UART complete event fails\r\n");
        return FALSE;
    }
    ...
    )
{
    ...
    if(dwPatchLen >= 240) //0xf0 = 240 
    {
        if(bt_send_data(hComPortFile, HCI_VS_PATCH_START, sizeof(HCI_VS_PATCH_START)) < 0){
            printf("Write patch start header fails\r\n");
            return FALSE;
        }
        ...
        if(bt_send_data(hComPortFile, ucTmpBuf, 240) < 0){	
            printf("Write patch start data fails\r\n");	
            return FALSE;
        }    
    ...
    while(dwPatchLen >= 240) //0xf0 = 240 
    {
        if(bt_send_data(hComPortFile, HCI_VS_PATCH_CONTINUE, sizeof(HCI_VS_PATCH_CONTINUE)) < 0){
            printf("Write patch continue header fails\r\n");
            return FALSE;
        }
        ...
        if(bt_send_data(hComPortFile, ucTmpBuf, 240) < 0){	
            printf("Write patch continue data fails\r\n");	
            return FALSE;
        }   
    ...
    if(dwPatchLen < 240) //0xf0 = 240
    {
        ...
        if(bt_send_data(hComPortFile, HCI_VS_PATCH_END, sizeof(HCI_VS_PATCH_END)) < 0){
            printf("Write patch end header fails\r\n");
            return FALSE;
        }
...
BOOL BT_ForceAlwaysSleep(HANDLE hComPortFile)
{
    ...
    if(bt_send_data(hComPortFile, HCI_VS_SET_ALWAYS_SLEEP, sizeof(HCI_VS_SET_ALWAYS_SLEEP)) < 0){    
        printf("Write always sleep command fails\r\n");
        return FALSE;
    }
...
BOOL BT_HCIReset(HANDLE hComPortFile)
{
    ...
    if(bt_send_data(hComPortFile, HCI_RESET, sizeof(HCI_RESET)) < 0){       
        printf("Write HCI reset command fails\r\n");
        return FALSE;
    }
    ...
    if(BT_ReadExpectedEvent(
        ...
        printf("Read reset complete event fails\n");
        return FALSE;
    }
